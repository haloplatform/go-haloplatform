...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Halos", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		DataDir: filepath.Join("node_data"),		
		P2P: p2p.Config{
			NAT:              nat.Any(),
			NoDiscovery:      true,			
			DiscoveryV5:      true,
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			signed, err := f.keystore.SignTx(f.account, tx, f.config.ChainId, true)

			log.Info("tx info", "tx", tx, "signed", signed, "error", err)			
