...
func RegisterRaftService(stack *node.Node, ctx *cli.Context, cfg gethConfig, ethChan <-chan *eth.Ethereum) {
	...
	electionTick := ctx.GlobalInt(utils.ElectionTickFlag.Name)
	maxTxsPerBlock := ctx.GlobalInt(utils.MaxTxsPerBlockFlag.Name)
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		rewardTime := time.Duration(rewardMinutes) * time.Minute
