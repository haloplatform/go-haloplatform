...
// GetPendingNumber returns number of all txs in pending map
func (pool *TxPool) GetPendingNumber() int {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	...
	for _, list := range pool.pending {
		cnt += len(list.txs.items)
	}

	return cnt
}

// PendingThrottled retrieves limited set of currently processable transactions
func (pool *TxPool) PendingThrottled(maxTxsPerBlock int, maxTxsPerAccount int) (map[common.Address]types.Transactions, error) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	...
	pending := make(map[common.Address]types.Transactions)
	for addr, list := range pool.pending {
		pending[addr] = list.FlattenNum(maxTxsPerAccount)
		txsCnt += len(pending[addr])

		if txsCnt > maxTxsPerBlock {
			break
		}
	}
	return pending, nil
...
func (pool *TxPool) ReplayRemoteTx(txs []*types.Transaction) {
	...
	for _, tx := range txs {
		...
		if pool.all[tx.Hash()] == nil {
			/// Validate again to prevent spammer
			/// TODO: if we have multi-tier MN, the lower layer do it only.
			if pool.validateTx(tx, false) == nil {
				/// Do broadcasting by raising event to txBroadcastLoop()
				go pool.txFeed.Send(TxPreEvent{tx})
			}
		}
	}
