...
// Glo: do not trace private transactions
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	// Glo: try to use private state to use it in Processor().Process()
	_, privateState, err := api.eth.blockchain.StateAt(start.Root())
	if privateState == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					res, err := api.traceTx(ctx, msg, vmctx, task.statedb, privateState, config)
					if err != nil {
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			_, _, _, _, err := api.eth.blockchain.Processor().Process(block, statedb, privateState, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	statedb, privateState, err := api.computeStateDB(parent, reexec)
	if err != nil {
		return nil, err
	}
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, privateState, api.config, vm.Config{})
...
// TODO: handle privateState as well as statedb
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, *state.StateDB, error) {
	...
	for block.NumberU64() < origin {
		...
		// Glo: temporary use statedb as privateState
	...
	log.Info("Historical state is not supported for PrivateState")
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	msg, vmctx, statedb, privateState, err := api.computeTxEnv(blockHash, int(index), reexec)
	if err != nil {
		return nil, err
	}
...
// be tracer dependent.
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb, privateState *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, statedb, privateState, api.config, vm.Config{Debug: true, Tracer: tracer})
