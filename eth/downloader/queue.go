...
func (q *queue) DeliverReceipts(id string, receiptList [][]*types.Receipt) (int, error) {
	...
	reconstruct := func(header *types.Header, index int, result *fetchResult) error {
		// Glo: need to exclude private receipts from receiptList before hash calculation
		// because header.ReceiptHash calculated only for public state, 
		// because private state can have different trie data in one block for different nodes
		var tmpPublicReceipts []*types.Receipt
		for _, receipt := range receiptList[index] {
			if !receipt.IsPrivate {
				tmpPublicReceipts = append(tmpPublicReceipts, receipt)
			}
		}

		if len(tmpPublicReceipts) == 0 || types.DeriveSha(types.Receipts(tmpPublicReceipts)) != header.ReceiptHash {
