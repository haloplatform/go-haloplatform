...
type Table struct {
	...
	///GLO: The current number of peers for checking FINDNODE
	peerCount *int32
}
...
type transport interface {
	...
	findnode(toid NodeID, addr *net.UDPAddr, target NodeID, peerCount int32) ([]*Node, error)
	close()
}
	...
	for {
		...
		for i := 0; i < len(result.entries) && pendingQueries < alpha; i++ {
			...
			if !asked[n.ID] {
				asked[n.ID] = true
				pendingQueries++
				go tab.findnode(n, targetID, reply, atomic.LoadInt32(tab.peerCount))
			}
		}
		if pendingQueries == 0 {
...
func (tab *Table) findnode(n *Node, targetID NodeID, reply chan<- []*Node, peerCount int32) {
	fails := tab.db.findFails(n.ID)
	r, err := tab.net.findnode(n.ID, n.addr(), targetID, peerCount)
	if err != nil || len(r) == 0 {
		fails++
...
/// Setting the current number of peers
/// Using the value for validating/accepting NEIGHBOR/V4 packet
func (tab *Table) SetPeerCount(peerCount int32) {
	atomic.StoreInt32(tab.peerCount, peerCount)
}
...
func (h *nodesByDistance) push(n *Node, maxElems int) {
	for _, node := range h.entries {
		if node.ID == n.ID {
			// GLO: Don't add a duplicated ID
			return
		}
	}
