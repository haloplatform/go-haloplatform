...
func ListenUDP(priv *ecdsa.PrivateKey, conn conn, realaddr *net.UDPAddr, nodeDBPath string, netrestrict *netutil.Netlist, networkId uint64) (*Network, error) {
	transport, err := listenUDP(priv, conn, realaddr, networkId)
	if err != nil {
		return nil, err
	}
	...
	log.Info("UDP for V5", "NetworkId", networkId)
	transport.net = net
	go transport.readLoop()
	return net, nil
}

func listenUDP(priv *ecdsa.PrivateKey, conn conn, realaddr *net.UDPAddr, networkId uint64) (*udp, error) {
	return &udp{conn: conn, priv: priv, networkId: networkId, ourEndpoint: makeEndpoint(realaddr, uint16(realaddr.Port))}, nil
}
...
func (t *udp) sendNeighbours(remote *Node, results []*Node) {
	...
	for i, result := range results {
		/// Don't include "remote" when sending to it
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	if err := decodePacket(buf, &pkt, t.networkId); err != nil {
...
func decodePacket(buffer []byte, pkt *ingressPacket, networkId uint64) error {
	...
	/// Only check for PING
	if err == nil && pkt.ev == pingPacket {
		pingPkt, ok := pkt.data.(*ping)
		if !ok {
			return fmt.Errorf("Bad ping package from: %v", fromID)
		}
		if pingPkt.NetworkId != networkId {
			return fmt.Errorf("Different networkId from %v: %d", fromID, pingPkt.NetworkId)
		}
	}
