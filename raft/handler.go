...
func NewProtocolManager(raftId uint16, raftPort uint16, blockchain *core.BlockChain, mux *event.TypeMux, bootstrapNodes []*discover.Node, joinExisting bool, datadir string, electionTick int, minter *minter, downloader *downloader.Downloader) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) startRaft() {
	...
	raftConfig := &etcdRaft.Config{
		...
		ID:            uint64(pm.raftId),
		ElectionTick:  pm.electionTick, // NOTE: cockroach sets this to 15
...
func (pm *ProtocolManager) addPeer(address *Address) {
	...
	pm.p2pServer.AddRaftPeer(address.nodeId)
...
func (pm *ProtocolManager) disconnectFromPeer(raftId uint16, peer *Peer) {
	...
	pm.p2pServer.RemoveRaftPeer(peer.p2pNode.ID)
			...
			for _, entry := range pm.entriesToApply(rd.CommittedEntries) {
				...
				case raftpb.EntryNormal:
					...
					if pm.blockchain.HasBlock(block.Hash(), block.NumberU64()) {
						...
						pm.eventMux.Post(core.NewVDTBBlockEvent{Block: &block})
